...
func ValidateSolveQuizRequest(request dto.SolveQuizRequest, questions []entity.Question) error {
	if request.QuizID == "" {
		return fmt.Errorf("%w: %s", ErrValidation, quizIdEmpty)
	}
...
// ValidateGetQuizzesByUserRequest validates the get quizzes by user request
func ValidateGetQuizzesByUserRequest(userId string, pageSize int) error {
	if userId == "" {

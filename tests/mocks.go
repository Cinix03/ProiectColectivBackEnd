...
func (m *MockQuizService) GetQuizzesByUser(userId string, pageSize int, lastKey string) ([]dto.ReadQuizResponse, string, error) {
	args := m.Called(userId, pageSize, lastKey)
	if args.Get(0) == nil {
		return nil, args.String(1), args.Error(2)
	}
